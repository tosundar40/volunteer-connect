...
const ModeratorOpportunities = () => {
  ...
  const fetchOpportunities = async (page = opportunityPage) => {
    try {
      ...
      if (opportunityCategory && opportunityCategory !== "all")
        params.category = opportunityCategory;
  ...
  useEffect(() => {
    fetchOpportunities(opportunityPage);
  }, [
    opportunityPage,
    opportunityQuery,
    opportunityStatus,
  ...
  const handleOpportunityKeyPress = (e) => {
    if (e.key === "Enter") handleOpportunitySearch();
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      setDeleting(true);
      await moderatorService.deleteOpportunityAsModerator(
  ...
  const confirmSuspendResume = async () => {
    ...
    try {
      ...
      if (actionType === "suspend") {
        await moderatorService.suspendOpportunity(
          selectedOpportunity.id,
          suspendReason
        );
        toast.success("Opportunity suspended");
      } else {
        await moderatorService.resumeOpportunity(
          selectedOpportunity.id,
          suspendReason
        );
      ...
      toast.error(
        error.response?.data?.message || `Failed to ${actionType} opportunity`
      );
    } finally {
      setSuspending(false);
    }
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      <Typography variant="h4" gutterBottom>
        <WorkIcon sx={{ mr: 1, verticalAlign: "middle" }} /> Opportunities
        Management
      </Typography>
            ...
            <Grid container spacing={2} alignItems="center">
              <Grid item xs={12} md={8}>
                <Typography variant="body2" color="text.secondary">
                  {opportunityTotal === 0
                    ...
                    : `Showing ${
                        (opportunityPage - 1) * opportunityPageSize + 1
                      }-${Math.min(
                        opportunityPage * opportunityPageSize,
                        opportunityTotal
                      )} of ${opportunityTotal} opportunities`}
                  {(opportunityQuery ||
              ...
              >
                {opportunityTotal > opportunityPageSize && (
                  <Typography variant="body2" color="text.secondary">
                    Page {opportunityPage} of{" "}
                    {Math.ceil(opportunityTotal / opportunityPageSize)}
                  </Typography>
                )}
              </Grid>
            </Grid>
                ...
                <TableRow key={opportunity.id}>
                  ...
                  <TableCell>
                    <Typography variant="body2">
                      {opportunity.charity?.organizationName || "N/A"}
                    </Typography>
                  </TableCell>
                  <TableCell>
                    <Chip
                  ...
                  <TableCell>
                    <Typography variant="body2">
                      {opportunity.volunteersConfirmed || 0}/
                      ...
                      {opportunity.status === "suspended" ? (
                        <Tooltip title="Resume Opportunity">
                          <IconButton
                            ...
                            color="success"
                            onClick={() =>
                              openSuspendDialog(opportunity, "resume")
                            }
                          >
                            <ResumeIcon />
                          </IconButton>
                        ...
                        <Tooltip title="Suspend Opportunity">
                          <IconButton
                            ...
                            color="warning"
                            onClick={() =>
                              openSuspendDialog(opportunity, "suspend")
                            }
                          >
                            <SuspendIcon />
                          </IconButton>
                  ...
                  <TableCell colSpan={7} align="center">
                    ...
                    >
                      ...
                      opportunityCategory !== "all"
                        ...
                        : "No opportunities found in the system."}
                    </Typography>
                  </TableCell>
                </TableRow>
              )}
        ...
        <Paper sx={{ mt: 3, p: 2 }} variant="outlined">
          <Grid container spacing={2} alignItems="center">
            <Grid item xs={12} md={6}>
              <Typography variant="body2" color="text.secondary">
                Total: {opportunityTotal} opportunities | Page {opportunityPage}{" "}
                of {Math.ceil(opportunityTotal / opportunityPageSize)}
              ...
              <Grid item xs={12} md={6}>
                <TextField
                  ...
                  onChange={(e) =>
                    setEditForm((prev) => ({
                      ...prev,
                      numberOfVolunteers: parseInt(e.target.value) || 1,
                    }))
                  }
                  inputProps={{ min: 1 }}
                  sx={{ mb: 2 }}
      ...
      >
        ...
        <DialogContent>
          <Alert
            severity={actionType === "suspend" ? "warning" : "info"}
            sx={{ mb: 2 }}
          >
            {actionType === "suspend"
        ...
        <DialogActions>
          ...
          <Button
            ...
            disabled={
              suspending || (actionType === "suspend" && !suspendReason.trim())
            }
          >
            {suspending
              ? "Processing..."
      ...
      >
        ...
        <DialogContent>
          <Alert severity="warning" sx={{ mb: 2 }}>
            This action will permanently delete the opportunity.
          </Alert>
          <Typography>
